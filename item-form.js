...
async function importCaptureFiles(fileList) {
  ...
  for (const file of jsonFiles) {
    ...
    if (data.type === 'receipt-capture') {
      ...
      const matchedImage = imageFiles.find((f) => f.name === data.imageFile);

      let title = 'Receipt (needs review — no image supplied)';
      let amount = 0;

      if (matchedImage) {
        try {
          const text = await runOCR(matchedImage);
          title = `${guessTitleFromText(text)} (OCR guess — please check)`;
          amount = guessAmountFromText(text);
        } catch (err) {
          ocrFailed++;
          title = 'Receipt (OCR failed — needs manual entry)';
        }
      }
  ...
  let message = `Imported ${imported} item(s).`;
  if (skipped > 0) message += ` Skipped ${skipped} unrecognised file(s).`;
  if (ocrFailed > 0) message += ` OCR failed on ${ocrFailed} — check title/amount manually.`;
  message += '\n\nAll amounts/titles from OCR are best-guesses — please verify each one.';
